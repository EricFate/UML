...
def get_class_mean(dataset, backbone_class, trainset):
    class_mean_path = './saves/stats/{}-{}-Mean.pth'.format(dataset, backbone_class)
    if osp.exists(class_mean_path):
        class_mean = torch.load(class_mean_path)
    else:
        ...
        class_mean = []
        with torch.no_grad():
            for i, batch in tqdm(enumerate(class_loader), total=len(class_loader), desc='Get Mean of SEEN Classes'):
        ...
        torch.save(class_mean, class_mean_path)
    return class_mean
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_path', type=str,
                        default='./checkpoints/best/Res12-pre.pth')  # './saves/initialization/miniimagenet/Res12-pre.pth'
    parser.add_argument('--gpu', type=str, default='0')
    parser.add_argument('--test_size', type=int, default=84)
    parser.add_argument('-e', '--num_test_episodes', type=int, default=5000)
    ...
    trainset = get_dataset(args.dataset, 'train', False, args)
    testset = get_dataset(args.dataset, 'test', False, args)
    ...
    # get SEEN-class mean
    class_mean = get_class_mean(args.dataset, args.backbone_class, trainset)

    # shot = [1, 5, 10, 20, 30, 50]
    # FSL Test, 1-Shot, 5-Way
    num_shots = [1, 5, 10, 20, 30, 50]
    test_acc_record_sim = np.zeros((args.num_test_episodes, len(num_shots)))
    ...
    for shot_ind, shot in enumerate(num_shots):
        few_shot_sampler = CategoriesSampler(testset.label, args.num_test_episodes, 5, shot + 15)
        ...
        with torch.no_grad():
            for i, batch in tqdm(enumerate(few_shot_loader), total=len(few_shot_loader),
                                 desc='%d-Shot 5-Way Test' % shot):
                if torch.cuda.is_available():
                    data, _ = [_.cuda() for _ in batch]
                else:
                    data = batch[0]
