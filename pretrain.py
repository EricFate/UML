...
def get_args():
    ...
    parser.add_argument('--backbone_class', type=str, default='SAN', choices=['ConvNet', 'ConvAttNet', 'Res12', 
                                                                                    'Res18', 'ResNest12', 'SAN'])
    ...
    parser.add_argument('--lr-scheduler', type=str, default='multistep', help='learning rate scheduler (default: cos)')    
    ...
    if args.lr_scheduler == 'cos':
        ...
        save_path2 = 'Bsz{}Epoch-{}-Step-lr{}-{}-{}decay{}'.format(args.batch_size, args.max_epoch, args.lr, args.step, args.gamma, args.weight_decay)
    elif args.lr_scheduler == 'multistep':
        save_path2 = 'Bsz{}Epoch-{}-MultiStep-lr{}-{}-{}decay{}'.format(args.batch_size, args.max_epoch, args.lr, args.step, args.gamma, args.weight_decay)
    else:
...
def get_loader(args):
    ...
    trainset = Dataset('train', args, augment=True, rand_aug=args.rand_aug)
    train_loader = DataLoader(dataset=trainset, batch_size=args.batch_size, shuffle=True, num_workers=4, pin_memory=True)
...
def get_optimizer(args, model, train_loader):
    ...
    if args.lr_scheduler == 'cos':
        ...
        lr_schedule = torch.optim.lr_scheduler.MultiStepLR(optimizer, [e  * len(train_loader) for e in eval(args.step)], args.gamma)
    else:
        raise ValueError('No Such Schedule') 
...
def train(args, model, train_loader, criterion, optimizer, lr_schedule, trlog):    
    ...
    for i, (data, label) in enumerate(train_loader, 1):
        ...
        if (i-1) % 100 == 0 or i == len(train_loader):
            if not args.mixup:
                print('epoch {}, train {}/{}, lr={:.5f}, loss={:.4f} acc={:.4f}'.format(epoch, i, len(train_loader), optimizer.param_groups[0]['lr'], loss.item(), acc))
            else:
                print('epoch {}, train {}/{}, lr={:.5f}, loss={:.4f}'.format(epoch, i, len(train_loader), optimizer.param_groups[0]['lr'], loss.item()))
