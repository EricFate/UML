...
def get_path(args):
    save_path1 = '-'.join(['FixRes', args.dataset, args.backbone_class, str(args.test_size)])
    ...
    if args.no_bn_wd:
        save_path2 += '-no-bn-wd'
    args.save_path = osp.join(args.save_dir, save_path1, save_path2)
...
if __name__ == '__main__':
    args = get_args()
    sizes = list(range(args.start, args.end + 1, 8))
    stats = np.zeros((len(sizes), args.max_epoch + 1))
    for i, args.test_size in enumerate(sizes):
        get_path(args)
        print('test size %d' % args.test_size)
        ...
        writer.close()
        stats[i, :] = trlog['val_acc_sim']
    os.makedirs('fig', exist_ok=True)
    plt.figure()
    for i in range(args.max_epoch + 1):
        print('finetune epoch {} best eval size {}'.format(i, sizes[np.argmax(stats[:, i])]))
        plt.plot(sizes, stats[:, i], label='epoch %d' % i)
    plt.legend()
    plt.ylabel('test accuracy')
    plt.xlabel('test resolution')
    plt.xticks(sizes)
    plt.grid(True)
    plt.savefig(osp.join('fig', 'size_acc.png'))
